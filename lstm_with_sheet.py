...
with open('aapldata.txt', 'r') as f:
    ...
    for row in f:
        ...
        for i in range(1, numvars + 1):
            if data[i].strip() != "":
                mat[-1].append(float(data[i].strip()))
                d[header[i-1]] += 1
            else:
                mat[-1].append(np.nan)
...
if not os.path.exists('data'):
    os.mkdir('data')
with open('data/AAPLimputed.txt', 'w') as f:
...
for epoch in (range(num_epochs)):
    width = random.randint(train_size/4, X_train_tensors_final.shape[1])
...
for i in range(len(test_predict)):
    if (test_predict[i])*(ytest[i]) >= 0:
        correct += 1
    if ytest[i] >= 0:
        increases += 1
print(correct, " out of ", len(test_predict), ", ", increases, " increases")
